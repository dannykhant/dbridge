...
//                    }
    ...
    private boolean isTriangleWithReturn() {
        ...
        if (getSuccRegions().contains(theSibling) || theSibling.getSuccRegions().contains(this))
            return false;
        // The sibling loops back to theOnlyPred (a back edge). This is a loop,
        // not a triangle-with-return: let the loop be formed first.
        if (theSibling.getSuccRegions().contains(theOnlyPred))
            return false;
